...
/// Singleton service that wires up Firebase Cloud Messaging for the passenger app.
class FcmService {
  FcmService._();
  ...
  static Future<void> initialize(SupabaseClient supabase) async {
    ...
    if (token != null) {
      debugPrint('[FcmService] FCM token (passenger): $token');
      await _saveTokenToSupabase(supabase, token);
    }

    // 3. Refresh token handler.
    messaging.onTokenRefresh.listen((newToken) async {
      debugPrint('[FcmService] FCM token refreshed (passenger): $newToken');
